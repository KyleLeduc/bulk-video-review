...
const defaultDigest = (): DigestFunction | null => {
  const subtle = globalThis.crypto?.subtle
  return subtle ? subtle.digest.bind(subtle) : null
}

export class FileHashGenerator {
  constructor(
    private readonly digest: DigestFunction | null = defaultDigest(),
  ) {}
  ...
  async generate(file: File): Promise<string> {
    ...
    const hashArray = Array.from(
      this.digest
        ? new Uint8Array(await this.digest('SHA-256', data))
        : sha256(data),
    )
