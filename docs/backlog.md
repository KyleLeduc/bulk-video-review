...
**Next:** Resolve the read/index pre-allocation bounds identified by the approved `mediabunny@1.55.7` [qualification audit](testing/mediabunny-qualification.md), then resume the [benchmark-only MP4/H.264 WebCodecs preview experiment](plans/2026-09-05-benchmark-webcodecs-design.md) and [implementation plan](plans/2026-09-05-benchmark-webcodecs.md). Approval was given on 2026-09-05; installation stopped at the source-resource gate. No new dependency was installed and WebCodecs/workers remain unimplemented. Upstream/library admission limits or revised experiment scope need an explicit decision; MP4Box.js remains an uninstalled follow-up, not assumed to pass the same gate. Normal-app enablement, native Edge/frame-image/decoder-memory acceptance and final integration remain open. This supersedes the encoding-worker-first sequence, not its eventual product safety requirements.
