...
**2026-09-07 fallback increment:** Owner reports four consistently failed clip loops with successful covers and approved motion clips → nine DOM stills → cover fallback. Implementation on the feature branch keeps fallback in the existing motion job, with original failure diagnostics and an independently versioned disposable cache product; no new scheduler or authoritative metadata writes. Seek priority, focus cancellation and completed-product reuse remain unchanged. See the [v3 smoke procedure](testing/ingestion-clip-ux-smoke.md#release-checkpoint--failed-clip-still-fallback-v3). Original file failures remain undiagnosed; native acceptance and master integration remain open.
...
- **2026-09-17 approved cache capacity:** increase generated-preview retention from 256 MiB to 1 GiB, with matching backup validation and a 2 GiB full-archive ceiling. Keep existing data, product recipes and oldest-used eviction. Scoped plan: [1 GiB cache](plans/2026-09-17-preview-cache-1gib.md). Automated boundary/reload verification and exact-build preprod release are required; owner large-library cache acceptance remains open. Original intermittent regeneration cause remains unproven.
