...
**Current experiment / next evidence:** The owner approved a [revised custom-file extraction-only scope](plans/2026-09-05-custom-mediabunny-benchmark.md) after the [source audit](testing/mediabunny-qualification.md), accepting its lack of hard parser-memory bounds. Pinned Mediabunny 1.55.7 is privately installed and the separate DOM/WebCodecs worker comparison is implemented; see [operator instructions](testing/custom-mediabunny-benchmark.md). No fixtures are needed on the owner PC. Collect native custom-file timing and image results next; no throughput win or normal-app enablement is claimed. MP4Box.js remains an uninstalled follow-up, not assumed to pass the same source gate. Native Edge/frame-image/decoder-memory acceptance, production safety requirements and final integration remain open.
