...
**Next:** Qualify the [benchmark-only MP4/H.264 WebCodecs preview experiment](plans/2026-09-05-benchmark-webcodecs-design.md), starting with the [demuxer decision](decisions/video-worker-demuxer.md) and [qualification implementation plan](plans/2026-09-05-benchmark-webcodecs.md). `mediabunny@1.55.7` is recommended but needs explicit owner approval before installation. WebCodecs/workers remain unimplemented. Normal-app enablement, native Edge/frame-image/decoder-memory acceptance and final integration remain open. This supersedes the encoding-worker-first sequence, not its eventual product safety requirements.
