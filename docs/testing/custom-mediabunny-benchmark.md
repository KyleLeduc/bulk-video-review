...
- For the reported immediate failures, rerun **Motion + seek quality** on the same nine files. Copy all JSON, including failures, and state local disk versus NAS. Clip and seek failure rows now retain allowlisted `diagnostics`: stage, codec, read counts, track start/end, player duration, time resolution and a `timelineReason` when the metadata guard rejects. Raw library errors, filenames and paths remain excluded; technical metadata can still identify a collection.
- The older **Three-second clips: smoke v1** skips the production player-timeline guard. On build `799ebbb`, the owner's legacy run passed eight files and hit `read-limit` on the ninth (426322427 bytes), whereas production motion plus three seek widths rejected all 36 attempts as `unsupported-timeline`. These are different execution paths, not evidence that 20 FPS caused a decoding regression. The exact guard branch for those files remains unconfirmed until the richer report is returned. This checkpoint **does not loosen the guard or repair files**.
- For speed, choose **Seek backends: DOM / Mediabunny · 160 px · 1 / 2 jobs · reversed passes** (`seek-backends-v1`). Start with a small representative folder. Eight configurations run on the retained selection: Mediabunny/1, DOM/1, Mediabunny/2, DOM/2, then the reverse sequence. Both use production `15s-max100` timestamp targets, JPEG quality .72, maximum width 160, and no upscaling. These are sampled seek images, not encoded key-packet extraction.
- Compare each complete configuration's `report.wallMs`, successful/expected frames and `peakActiveJobs`, then visually compare video 1's two samples. Failures are not speed wins. Player metadata preparation is inside both backends' file/batch timing and 120-second file deadline. Native reads are unknown (`null`) and uncapped; Mediabunny reads use the existing buffered 1 MiB reader and 1 GiB budget. Both cap encoded output at 16 MiB per file; only the first-pass, one-job video-1 samples are retained (two sets, at most 32 MiB). These are not decoder-memory limits.
- Keep the tab visible. Stop/tab hiding aborts active jobs, leaves undispatched files unrun and retains partial numeric results; no samples are published for an interrupted plan. Reversed passes do not reset browser/OS/NAS caches. Folder selection makes the source mix realistic, but this still excludes identification, persistence and ingestion-queue scheduling.

The sections below retain earlier recipes and historical measurements; the current default is **Motion + seek quality**, not the legacy three-second smoke.

Checkpoint implementation checks (2026-09-11): lint, application/Cypress types, production build and all 780 unit tests passed. Isolated Chrome 152 and Edge 152 each passed both automatic-plan browser tests, including nested duplicate basenames, ignored sidecars, redacted exports, all eight seek configurations, first-frame blue pixels for both backends, and existing clip playback/cycling. Review identified a native initial-frame race: even `readyState=4` could draw an empty paused frame until an explicit seek to the unchanged zero timestamp. The benchmark now waits for that initial seek and frame readiness, with bounded cancellation/cleanup. Production media primitives and timeline policy are unchanged. These synthetic checks do not establish owner-file compatibility or a performance winner.

### Manual still comparison
