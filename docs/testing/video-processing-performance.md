...
4. Click **Copy JSON** and paste the report into the conversation, or use **Download JSON**. Both contain the same complete evidence. If clipboard access is unavailable or denied, the page opens and selects the readonly JSON: press Ctrl+C (Cmd+C on Mac). Failed, hidden-tab, incomplete, mixed-identity or unresolved-cleanup results are retained but excluded from comparable summaries. Memory/native decoder and interaction latency are unavailable in the page; overlapping phase totals are not percentages of wall time.
...
### Interpreting repetitions and faster fresh runs

Each repetition/configuration gets a new temporary pair database. Its fresh trial and optional cached trial use separate frames; the cached trial intentionally retains the fresh trial's database. The parent waits for host closure and database deletion before admitting the next pair, and stops on uncertain cleanup. This is **fresh application storage**, not a restarted browser or cleared browser/OS cache. The same selected File objects remain available across repetitions.

A sequence such as 25/17/13/10/10 seconds could reflect warming outside the app database, but elapsed times alone cannot establish the cause. Paste the full JSON, preserving all repetitions. For every fresh row, check `foreground.counts.created` against successful output videos, `existing: 0`, completed preview attempts/frames, phase sample counts, and `cleanup: complete`; compare seek/metadata/persistence times across repetitions. Equal work counts with shorter waits support faster repeated processing, not proof of any particular browser/OS cache. Do not silently discard early runs or present cached-row times as faster decoding. A later method comparison needs matched conditions and explicitly separated warmup/first-use results.
