...
- Use `npm run worktree` as the canonical worktree CLI; in an interactive terminal with no subcommand it opens the launcher.
- Create new worktrees with `npm run worktree -- add <branch> [path]` instead of raw `git worktree add`.
- If entering an existing or manually created worktree, run `npm run worktree -- bootstrap [path]` before `npm` or dev commands. With no path it bootstraps the current checkout/worktree.
...
- Main commands: `npm run dev`, `npm run worktree -- [subcommand]`, `npm run dev:worktree`, `npm run dev:status`, `npm run dev:stop`, `npm run worktree:add -- <branch> [path]`, `npm run worktree:bootstrap -- [path]`, `npm run lint`, `npm run type-check`, `npm run test:unit`, `npm run test:ci`, `npm run test:e2e`, `npm run build`, `npm run dep-graph`, `npm run diagram:video`
